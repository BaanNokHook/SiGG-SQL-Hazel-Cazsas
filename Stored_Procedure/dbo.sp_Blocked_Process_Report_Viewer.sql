...
IF OBJECT_ID('sp_blocked_process_report_viwer') IS NULL   
    EXEC ('  
    CREATE PROCEDURE dbo.sp_blocked_process_report_viwer    
    AS 
    SELECT ''Replace Me''')   
GO  
...
-- Validate @Type  
IF (@Type NOT IN ('FILE', 'TABLE', 'XMLFILE', 'XESSION'))   
...
IF (@Type = 'XESSION' AND NOT EXISTS (  
    SELECT *   
    FROM sys.server_event_session es     
