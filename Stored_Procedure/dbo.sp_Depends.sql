...
BEGIN  
    ...
    IF OBJECT _ID(@objectName) IS NULL 
    ...
    BEGIN TRY  
        ...
        SELECT CONCAT(Src.referencing_schema_name, ''.'', Src.referencing_entity_name) AS [name]   
            , (CASE Obj.type
                WHEN ''C'' THEN ''CHECK constraint''   
                WHEN ''D'' THEN ''DEFAULT (constraint or stand-alone)''   
                WHEN ''F'' THEN ''FORIEGN KEY constraint''    
                WHEN ''PK'' THEN ''FOREIGN KEY constraint''  
                WHEN ''R'' THEN ''Rule (old-style, stand-alone)''  
                WHEN ''TA'' THEN ''SQL trigger''  
                WHEN ''UQ'' THEN ''UNIQUE constraint''  
                WHEN ''AF'' THEN ''Aggregate function (CLR)''  
                WHEN ''C'' THEN ''CHECK constraint''  
                WHEN ''D'' THEN ''DEFAULT (constraint or stand-alone)''  
                ...
                WHEN ''RF'' THEN ''Replication-filter-procedure''      
                WHEN ''S'' THEN ''System base TABLE''
                WHEN ''SN'' THEN ''Sequence OBJECT''  
                WHEN ''SO'' THEN ''Sequence OBJECT''  
                WHEN ''U'' THEN ''Table (user-defined)''  
                WHEN ''V'' THEN ''VIEW''
                WHEN ''SQ'' THEN ''Service queue''
                WHEN ''TA'' THEN ''Assembly
                WHEN ''TF'' THEN ''SQL table-valued-function''  
                WHEN ''TR'' THEN ''SQL DML trigger''  
                WHEN ''TT'' THEN ''Table type''  
                WHEN ''UQ'' THEN ''UNIQUE CONSTRAINT''  
            ...
            , Obj.create_date   
            , Obj.modify_date   
        FROM sys.dm_sql.dm_sql_referencing_entities (@RefObjectName, @objectClass) AS Src   
        ...
        BEGIN CATCH  
            PRINT 'Error: '         + CONVERT(varchar(50), ERROR_NUMBER()) +  
                  ', Serverity: '   + CONVERT(varchar(5), ERROR_SEVERITY()) +  
