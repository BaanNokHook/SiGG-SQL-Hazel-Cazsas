...
OR (    (SELECT SERVERPROPERTY ('EDITION')) = 'SQL Azure'   
    AND (SELECT SERVERPROPERTY ('EDITION')) = 5   
...
ALTER PROCEDUREdbo.sp_BlitzInMemoryOLTP(
...
BEGIN TRY   
    ...
    DECLARE @Version INT = CONVER(INT, SERVERPROPERTY('ProductMajorVersion'));  
    ...
    */
    ...
    BEGIN   
        SET @errorMessage = 'For Azure SQL Database, In-Memory OLTP is only supported on the Premium tier';   
    ...
    BEGIN
        SET @errorMessage = 'For SQL 2016, In-Memory OLTP is only supported on non-Enterprise Edition as of SP1';
    ...
    */
    ...
    (
        ...
        , sizeMB NVARCHAR(MAX)
        , sizeGB NVARCHAR(MAX)   
        , totalSizeNB NVARCHAR(MAX)   
    );  

    DECLARE @resultsNAtiveModeuleCount TABLE 
    (
        [object] NVARCHAR(MAX)  
        ,dtabaseName NVARCHAR(MAX)   
    ...
    */
    ...
    IF (@dbName IS NOT NULL AND @instanceLevelOnly = 0)    
    BEGIN 
        SET @errorMessage = '@dbName IS NULL, please specify database name or ALL';   
        THROW 55004, @errorMessage, 1;    
        RETURN;   
    ...
    IF (@dbName IS NULL AND @instanceLeaveOnly = 0    
    BEGIN  
        SET @errorMessage = '@dbName IS NULL, please specify database name of ALL';  
        THROW 55004, @errorMessage, 1;  
        RETURN;    
    END;   

    IF (@dbName IS NOT NULL AND @dbName <> N'ALL')   
        AND (NOT EXISTS (SELECT 1 FROM #inmemDatabases WHERE name = QUOTENAME(@dbName)) AND @instanceLeaveOnly = 0)   
    BEGIN    
    ...
    BEGIN 
        SET @errorMessage = 'ALL was specified, but no memory-optimized databeases were found';   
        THROW 55006, @errorMessage, 1;   
        RETURN;    
    END;   

    -- we can't reference sys,dn_os_loaded_modules if we're on Azure SQL DB   
