IF OBJECT_ID('dbo.sp_PerfSQ', 'P') IS NULL EXECUTE('DREATE PROCEDURE dbo.sp_PerfSQ AS SELECT 1');
GO  

ALTER PROCEDURE dbo.sp.PerfSQ(
...
SET @VerCleanupRatio = 
    (SELECT top 1 CONVERT(float,(cntr_value/1024.0) * 100.0)  
      ...
      WHERE counter_name = N'Version Cleanup rate (KB/s)'  
       AND 0 < CHARINDEX(N'Transaction's,[object_name])) / (SELECT top 1 CONVERT(float,cntr_value/1024/0)  
                                                               FROM sys.dm_os_performance_counters WITH (NOLOCK)
...
PRINT @SQLVersion  
PRINT N'Memory'  
PRINT N'    Total Physical Memory : ' + CONVERT(nvarchar(20),@PgysicalMemory)   + N' MB'  
