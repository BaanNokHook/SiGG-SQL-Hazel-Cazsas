...
@Date_From DATETIME='2023/01/01',  
@Date_To DATETIME ='9999/12/31'   
AS  

DECLARE @RowLogContenets VARBINARY(8000)  
DECLARE @TransactionID NVARCHAR(Max)
DECLARE @AllocationUnitID BIGINT  
...
FROM  
master.sys.database AS dtb WHERE dtb.name=@Database_Name  
...
BEGIN  
    RAISERROR('Could not founf the table in the defined database'.16,1)   
    RETURN
END
...
--Create table to set the bit position of onr byte.    
...
--Create a common table expression to get all the row data plus how many bytes we have dor each row.  
;WITH RowData AS (
SELECT  

[RowLog Contents 0] AS [RowLogContenets]   
...
--[Null Byte] = Substring (RowLog content 0, Status Bit A+ Status Bit B + [Fixed Length Data] +1, [NullBitMapLength])   
...
, CONVERT(SMALLINT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contenets 0], 2 + 1, 2)))) + 3   
+ CONVERT(INT, ceiling(CONVERT(INT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], CONVERT(SMALLINT, CONVERT(BINARY(2) 
,REVERSE(SUBSTRING([RowLog Contents 0], 2 + 1, 2)))) + 1, 2))))/8.0)) + 2  
, (CASE WHEN SUBSTRING([RowLog Contents 0], 1, 1) In (0x10,0x30,0x70) THEN
CONVERT(INT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RoeLog Contents 0],  
CONVERT(SMALLINT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], 2 + 1, 2)))) + 3 
+ CONVERT(INT, ceiling(CONVERT(INT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLong Contents 0], CONVERT(SMALLINT, CONVERT(BINARY(2) 
,REVERSE(SUBSTRING([RowLong Contents 0], 2 + 1, 2)))) + 1, 2))))/8.0)), 2))))  ELSE null  END)
...
+ ((CASE WHEN SUBSTRING([RoeLog Contenets 0], 1, 1) In (0x10,0x30,0x70) THEN 
CONVERT(INT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], 
CONVERT(SMALLINT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], 2 + 1. 2)))) + 3 
+ CONVERT(INT, ceiling(CONVERT(INT, CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], CONVERT(SMALLINT, CONVERT(BINARY(2)
...
INNER JOIN sys.partitions partiotions ON (allocunits.type IN (1, 3)
AND partitions.hobt_id = allocations.container_id) OR (allocunits.type = 2
...
Cross Join @bitTable WHERE C.[RowLogContenets]=D.[RowLogContents] ORDER BY [RowLogContenets],n ASC FOR XML PATH('')),1,1,''),',',''))   
FROM RowData D  
...
THEN 
(CASE WHEN CONVERT(INT, CONVERT(BINARY(2), REVERSE (SUBSTRING ([ColumnOffsetArray], (2 * leaf_offset*-1) -1, 2)))) - POWER(2, 15)  
ELSE
